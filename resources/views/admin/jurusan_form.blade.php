...
@section('title', (isset($data) ? 'Edit' : 'Tambah') . ' Jurusan')
@section('page_title', (isset($data) ? 'Edit' : 'Tambah') . ' Jurusan')
...
<div class="max-w-2xl bg-white shadow-sm rounded-2xl overflow-hidden border border-slate-100 p-6">
    <form action="{{ isset($data) ? route('jurusan.update', $data->id) : route('jurusan.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="kode_jurusan" value="{{ old('kode_jurusan', $data->kode_jurusan ?? '') }}" class="w-full rounded-lg border-slate-300 border px-4 py-2 focus:ring-brand-500 focus:border-brand-500" required>
        </div>
        ...
        <div class="mt-6 flex gap-3">
            ...
            <a href="{{ route('jurusan.index') }}" class="px-6 py-2.5 bg-slate-100 text-slate-700 font-semibold rounded-xl hover:bg-slate-200 transition">Batal</a>
        </div>
    </form>
</div>
@endsection