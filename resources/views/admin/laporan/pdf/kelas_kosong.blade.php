...
<head>
<meta charset="UTF-8">
<title>Laporan Kelas Kosong</title>
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .kop-logo { width: 60px; height: 60px; background: #1B2A4A; border-radius: 8px; color: white; font-size: 20px; font-weight: 700; text-align: center; line-height: 60px; flex-shrink: 0; }
    ...
    tbody td { padding: 6px 6px; font-size: 9px; border-bottom: 1px solid #e2e8f0; }
    ...
    .badge-kosong           { background: #fee2e2; color: #991b1b; }
    .badge-terlaksana       { background: #d1fae5; color: #065f46; }
    .badge-tidak-terlaksana { background: #fee2e2; color: #991b1b; }
...
<body>
    ...
    <div class="laporan-judul">
        <h2>Laporan Kelas Kosong / Tidak Terlaksana</h2>
    ...
    <div class="info-filter">
        ...
        @endif
        <div class="info-item">Total Data: <span>{{ $data->count() }} kelas kosong</span></div>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $row->guru ?? '-' }}</td>
                <td>
                    @php $st = strtolower(str_replace(' ', '-', $row->status ?? 'kosong')); @endphp
                    <span class="badge badge-{{ $st }}">{{ $row->status ?? 'Kosong' }}</span>
                </td>
            ...
            <tr>
                <td colspan="8" style="text-align:center; color:#888; padding: 20px;">
                    Tidak ada kelas kosong pada periode ini.
                </td>
            </tr>
            @endforelse
        </tbody>
