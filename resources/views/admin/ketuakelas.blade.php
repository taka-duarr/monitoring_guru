...
@section('title', 'Ketua Kelas - Monitoring Guru')
@section('page_title', 'Data Ketua Kelas')
...
<div class="bg-white shadow-sm rounded-2xl overflow-hidden border border-slate-100">
    <div class="p-6 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
            <h3 class="text-lg font-bold text-slate-800">Daftar Ketua Kelas</h3>
            <p class="text-sm text-slate-500 mt-0.5">Total: <strong>{{ $data->total() }}</strong> data</p>
        </div>
        <a href="{{ route('ketuakelas.create') }}" class="inline-flex items-center px-4 py-2 bg-brand-600 text-white rounded-xl text-sm font-medium hover:bg-brand-700 transition-colors shadow-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                        ...
                        <form method="POST" action="{{ route('ketuakelas.destroy', $row->id) }}" class="inline" onsubmit="return confirm('Hapus data ini?')">
                            @csrf @method('DELETE')
                ...
                <tr>
                    <td colspan="100%" class="px-6 py-16 text-center text-slate-400">
                        ...
                        <p class="font-semibold text-slate-500">Belum ada data Ketua Kelas</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
