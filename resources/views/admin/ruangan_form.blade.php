...
@section('title', (isset($data) ? 'Edit' : 'Tambah') . ' Ruangan')
@section('page_title', (isset($data) ? 'Edit' : 'Tambah') . ' Ruangan')
...
<div class="max-w-2xl bg-white shadow-sm rounded-2xl overflow-hidden border border-slate-100 p-6">
    <form action="{{ isset($data) ? route('ruangan.update', $data->id) : route('ruangan.store') }}" method="POST">
        ...
        <div class="mt-6 flex gap-3">
            ...
            <a href="{{ route('ruangan.index') }}" class="px-6 py-2.5 bg-slate-100 text-slate-700 font-semibold rounded-xl hover:bg-slate-200 transition">Batal</a>
        </div>
    </form>
</div>
@endsection