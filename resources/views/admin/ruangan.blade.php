...
@section('title', 'Ruangan - Monitoring Guru')
...
<div class="bg-white shadow-sm rounded-2xl overflow-hidden border border-slate-100">
    <div class="p-6 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        ...
        <a href="{{ route('ruangan.create') }}" class="inline-flex items-center px-4 py-2 bg-brand-600 text-white rounded-xl text-sm font-medium hover:bg-brand-700 transition-colors shadow-sm">
        ...
        <table class="w-full">
            <thead class="bg-slate-50 border-b border-slate-100">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Nama Ruangan</th>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                        <a href="{{ route('ruangan.edit', $row->id) }}" class="text-blue-600 hover:text-blue-800 font-semibold">Edit</a>
                        <form method="POST" action="{{ route('ruangan.destroy', $row->id) }}" class="inline" onsubmit="return confirm('Hapus data ini?')">
                            @csrf @method('DELETE')
                ...
                <tr>
                    <td colspan="100%" class="px-6 py-16 text-center text-slate-400">
                        ...
                        <p class="font-semibold text-slate-500">Belum ada data Ruangan</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
