...
@section('title', 'Manajemen Jurusan - SIMGURU')
...
<div x-data="{ 
        ...
        <div class="d-flex align-center gap-2">
            ...
            <div class="position-relative" @click.away="exportDropdownOpen = false">
                ...
                <div class="card shadow-md p-1" 
                    ...
                    <a href="{{ route('jurusan.export', array_merge(request()->query(), ['format' => 'pdf'])) }}" 
                    ...
                    <a href="{{ route('jurusan.export', array_merge(request()->query(), ['format' => 'excel'])) }}" 
                       class="d-flex align-center gap-2 px-2 py-2 text-sm text-neutral-700 hover:bg-neutral-50 rounded"
                       style="text-decoration: none;">
    ...
    <div class="table-wrapper card p-0 overflow-hidden">
        ...
        @if($data->isEmpty())
            <div class="table-empty-state">
                ...
                <span class="table-empty-title">Tidak ada data jurusan</span>
                <span class="table-empty-sub">Belum ada jurusan yang terdaftar.</span>
                ...
                <tbody>
                    @foreach($data as $row)
                        <tr>
                            ...
                            <td>
                                <span class="badge badge-info">{{ $row->kode_jurusan }}</span>
                            </td>
                            <td class="col-actions col-center">
                                <div class="action-buttons-group">
                                    <!-- Edit Record -->
                                    <x-tooltip text="Edit Data">
                                        <a href="{{ route('jurusan.edit', $row->id) }}" class="btn btn-ghost action-edit">
                                    ...
                                    <x-tooltip text="Hapus Data">
                                        <button type="button" class="btn btn-ghost action-delete" 
                                                @click="$dispatch('confirm-delete', {
                                                    url: '{{ route('jurusan.destroy', $row->id) }}',
                                                    name: 'Jurusan {{ addslashes($row->name) }}'
                                                })">
    ...
    <div class="modal-backdrop" 
         ...
         style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background-color: rgba(15, 23, 42, 0.6); backdrop-filter: blur(4px); z-index: 100; display: flex; align-items: center; justify-content: center;">
        ...
        <div class="card bg-white shadow-xl" 
            ...
            <div class="d-flex align-center justify-between pb-3 border-b border-neutral-100 mb-4">
                <h3 class="text-lg font-bold text-primary-900">Import Data Jurusan</h3>
            ...
            <!-- Form -->
            <form action="{{ route('jurusan.import') }}" method="POST" enctype="multipart/form-data" x-data="{ importing: false }" @submit="importing = true">
                @csrf
                
                <div class="d-flex flex-column gap-4">
                    <p class="text-sm text-neutral-600">
                        Unggah berkas data jurusan Anda dalam format <strong>Excel (.xlsx)</strong> atau <strong>CSV</strong>. Silakan unduh template di bawah untuk melihat kolom yang wajib diisi.
                    </p>

                    <!-- Template Link -->
                    <a href="{{ route('jurusan.import.template') }}" class="d-inline-flex align-center gap-2 text-sm text-primary font-semibold hover:underline" style="text-decoration: none;">
                    ...
                    <div class="mt-2 text-xs text-neutral-400">
                        * Catatan: Nama Jurusan harus unik dan tidak boleh ada yang sama.
                    </div>
                </div>
